...
- Первый экран: темная industrial-сцена, крупный B2E header, телефонный блок как в референсе, CTA, hero RFQ-панель, сжатые производственные proof points и workflow-линия.
- Сразу после hero идут инженерный контекст расчета, `Пакет КП`, факторы КП, маршруты заявки, сценарии заказчика, светлый `Каталог продукции` из 8 карточек, затем услуги, компания, производство, инженерный блок с письмами, клиенты, контакты и footer.
...
- Каталог: публичные CTA `Скачать каталог` и `Смотреть весь каталог` ведут на подготовленный PDF-каталог.
- Заявка: быстрый расчет, сценарии заказчика, `Пакет КП`, факторы КП и маршруты заявки открывают универсальную форму с заранее заполненным типом проекта и контекстом задачи. Внутри формы есть контекстные подсказки по пакету исходников, кнопки добавления вводных в описание, readiness-индикатор по исходникам, объему, сроку/адресу, динамический следующий шаг после отправки и готовый бриф для копирования или email-черновика.
...
Browser proof should cover desktop `1536x1024`, laptop `1366x900`, and mobile `390x844`: hero, quote factors, catalog, services, production, callback popup, lead brief copy/email fallback, map, footer, no horizontal overflow, all generated assets loaded, 4 map locations and clickable coverage mode available.
